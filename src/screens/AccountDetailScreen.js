import { 
  View, Text, ScrollView, TextInput, StyleSheet, Modal, TouchableOpacity, Share, Linking 
} from "react-native";
...
export default function AccountDetailScreen({ route, navigation }) {
  // 1. GET REAL USER FROM PARAMS (Sent from App.js)
  const { account: initialAccount, user } = route.params;
  ...
  async function loadPayments() {
    try {
      // 2. PASS BUSINESS ID (Mandatory for Security Rules)
      const data = await fetchPaymentsForAccount(account.id, user.businessId);
      setPayments(data);
  ...
  async function addPayment() {
    ...
    const paymentData = {
      ...
      isVerified: method === "cash", // Cash is auto-verified, M-Pesa is false
    };

    try {
        // 3. SAVE USING REAL USER OBJECT
        await createPayment(paymentData, user);
        ...
        setReceiptData({
          businessName: user.businessName || "Official Receipt",
          account: { 
            type: account.type, 
        ...
        alert("Error saving payment. Check your connection.");
    }
  }
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={styles.statsRow}>
          <View>
            ...
            <Text style={styles.value}>KES {account.totalAmount.toLocaleString()}</Text>
          </View>
          <View>
            <Text style={styles.label}>Remaining</Text>
            <Text style={[styles.value, { color: "red" }]}>KES {balance.toLocaleString()}</Text>
          </View>
        </View>
        ...
        <View style={styles.historySection}>
          ...
          {payments.map(p => (
            <View key={p.id} style={styles.historyCard}>
              <View style={{ flex: 1 }}>
                <Text style={{ fontWeight: 'bold' }}>KES {p.amount.toLocaleString()}</Text>
                <Text style={{ color: '#666', fontSize: 12 }}>
                  {p.createdAt?.seconds 
                    ? new Date(p.createdAt.seconds * 1000).toLocaleDateString() 
                    : "Today"} • {p.paymentMethod.toUpperCase()}
                </Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  historyCard: { backgroundColor: '#fff', padding: 12, borderRadius: 8, marginBottom: 8, borderLeftWidth: 4, borderLeftColor: '#1565c0', flexDirection: 'row', alignItems: 'center' },
  ...
  shareRow: { flexDirection: 'row', justifyContent: 'space-around', marginTop: 15 },
